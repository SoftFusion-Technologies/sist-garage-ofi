...
export default function ModalMediosPago({
  ...
  // Guardar o editar
  const guardar = async () => {
    if (String(nuevo.tipo).toUpperCase() === 'COMPUESTO') {
      nuevo.ajuste_porcentual = 0;
    }
  ...
  const isCompuesto =
    String(nuevo.tipo || 'SIMPLE').toUpperCase() === 'COMPUESTO';

  // si está editando uno compuesto ya guardado, no permitir bajarlo a SIMPLE desde acá
  const bloquearCambioATipoSimple =
    !!editando &&
    String(editando.tipo || 'SIMPLE').toUpperCase() === 'COMPUESTO';

  const toggleTipo = () => {
    // si está bloqueado, no hacemos nada
    // if (bloquearCambioATipoSimple) {
    //   Toast.fire({
    //     icon: 'info',
    //     title:
    //       'Este medio es compuesto. Editá sus componentes desde “Combinar Medios”.'
    //   });
    //   return;
    // }
    ...
    setNuevo((prev) => {
      const nextTipo =
        String(prev.tipo || 'SIMPLE').toUpperCase() === 'SIMPLE'
          ? 'COMPUESTO'
          : 'SIMPLE';
      ...
      return {
        ...prev,
        tipo: nextTipo,
        // si pasa a COMPUESTO, forzamos ajuste 0
        ajuste_porcentual: nextTipo === 'COMPUESTO' ? 0 : prev.ajuste_porcentual
      };
    });
  };
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {/* Badge Tipo (SIMPLE / COMPUESTO) */}
                          <span
                            className={`text-[10px] px-2 py-0.5 rounded-full border font-mono ${
                              String(m.tipo || 'SIMPLE').toUpperCase() ===
                              'COMPUESTO'
                            ...
                            title={
                              String(m.tipo || 'SIMPLE').toUpperCase() ===
                              'COMPUESTO'
                                ? 'Medio compuesto (tiene componentes)'
                                : 'Medio simple'
                            }
                          >
                            {String(m.tipo || 'SIMPLE').toUpperCase()}
                  ...
                  <div className="px-5 sm:px-7 py-4 flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:justify-end mt-2">
                      ...
                      <button
                        type="button"
                        onClick={toggleTipo}
                        className={`inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-semibold border transition
      ${
        isCompuesto
          ? 'border-cyan-500/60 bg-cyan-500/10 text-cyan-200 hover:bg-cyan-500/15'
          : 'border-zinc-700 bg-zinc-900 text-zinc-200 hover:bg-zinc-800'
      }`}
                        title="Tipo de medio (SIMPLE o COMPUESTO)"
                      >
                        <span className="text-[11px] uppercase tracking-[0.14em]">
                          Tipo
                        </span>
                        <span className="font-extrabold">
                          {isCompuesto ? 'COMPUESTO' : 'SIMPLE'}
                        </span>
                        <span
                          className={`h-5 w-10 rounded-full relative border transition ${
                            isCompuesto
                        ...
                        >
                          <span
                            className={`absolute top-1/2 -translate-y-1/2 h-4 w-4 rounded-full transition ${
                              isCompuesto
                                ? 'left-[22px] bg-cyan-300'
                                : 'left-[2px] bg-zinc-300'
                            }`}
                          />
                        </span>
                      </button>
